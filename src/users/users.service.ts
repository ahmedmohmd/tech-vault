...
export class UsersService {
	...
	public async getAllUsers({
		...
		if (page) {
			allUsers.skip((Number(page) - 1) * Number(limit));
		}

		if (limit) {
			allUsers.limit(Number(limit));
		}
	...
	public async createUser(
		...
		try {
			...
			if (userData.phoneNumber) {
				createdPhoneNumber = this.phoneRepository.create({
			...
			let createdAddress;
			if (userData.city && userData.country && userData.postCode) {
				createdAddress = this.addressRepository.create({
					city: userData.city,
	...
	public async updateUser(
		...
		if (userImage) {
			...
			updatedUser.userImage = targetImage;
		}
	...
	public async makeEmailPrimary(userId: number, email: string) {
		...
		if (currentPrimaryEmailId) {
			currentPrimaryEmail.isPrimary = false;
		}
